...
3. **下载**:约 5~10 分钟后,在 **Actions 产物(Artifacts)** 或 **Releases** 页面拿到 `DeepSeek-Harness-Setup-0.1.0.exe`(约 300MB,内嵌完整 Node 24 运行时和 dsh),发给任何人双击即可安装,全程不需要对方装任何环境。
...
- **首次启动慢**:dsh 首次运行要初始化插件树,启动画面最长等待 2 分钟,之后正常秒开;
