...
- **第一条命令卡几分钟**(≤0.1.2 问题):dsh 的 Windows 沙箱(workspace-write 默认模式)首次受限执行时会对**整棵工作区目录树**递归传播 ACL 权限,文件越多越慢(上游已知代价)。0.1.2 及之前工作区误设为运行时目录(含数万个文件),0.1.3 起工作区改为独立的空目录 `C:\Users\<用户名>\DeepSeekHarness`,首次传播秒级完成,无需切换到"完全访问"模式;
- **Agent 的工作目录在哪**:`C:\Users\<用户名>\DeepSeekHarness`,让 Agent 读写文件默认都在这里;
